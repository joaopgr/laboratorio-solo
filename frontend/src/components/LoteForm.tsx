...
const loteSchema = z.object({
  ...
  tipoAnalise: z.enum(['solo', 'foliar']).default('solo'),
...
export function LoteFormNew({ lote, clienteId, isOpen, onClose }: LoteFormProps) {
  ...
  const tipoAnalise = watch('tipoAnalise')
  
  // Quando o tipo de análise mudar para foliar, selecionar automaticamente os campos específicos
  useEffect(() => {
    if (tipoAnalise === 'foliar' && !lote) {
      // Só aplica para novos lotes (não editando)
      setValue('rotina', true)
      setValue('micronutrientes', true)
      setValue('enxofre', true)
      setValue('nitrogenio', true)
      setValue('organica', false)
      setValue('prem', false)
      setValue('granulometria', false)
    }
  }, [tipoAnalise, setValue, lote])

  useEffect(() => {
    if (isOpen) {
      const isFoliar = lote?.tipoAnalise === 'foliar'
      ...
      reset({
        ...
        // Tipos de análise (solo) - para foliar, alguns vêm selecionados por padrão
        rotina: lote?.rotina || (isFoliar ? true : false),
        organica: lote?.organica || false,
        micronutrientes: lote?.micronutrientes || (isFoliar ? true : false),
        enxofre: lote?.enxofre || (isFoliar ? true : false),
        prem: lote?.prem || false,
        nitrogenio: lote?.nitrogenio || (isFoliar ? true : false),
  ...
  const onSubmit = async (data: LoteFormData) => {
    try {
      const loteData: CreateLoteAmostraData = {
        ...data,
        clienteId,
      }
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e) => setValue('rotina', e.target.checked)}
                    />
                    <label htmlFor="rotina-foliar" className="ml-2 block text-sm text-gray-700">
                      Rotina (R$ 15,00)
                    </label>
                  </div>
                  ...
                  <div className="flex items-center">
                    <input
                      type="checkbox"
                      id="micronutrientes-foliar"
                      className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                      checked={watch('micronutrientes') || false}
                      onChange={(e) => setValue('micronutrientes', e.target.checked)}
                    />
                    <label htmlFor="micronutrientes-foliar" className="ml-2 block text-sm text-gray-700">
                      Micronutrientes (R$ 15,00)
                    </label>
                  </div>
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e) => setValue('enxofre', e.target.checked)}
                    />
                    <label htmlFor="enxofre-foliar" className="ml-2 block text-sm text-gray-700">
                      Enxofre (R$ 15,00)
                    </label>
                  </div>
                  ...
                  <div className="flex items-center">
                    <input
                      type="checkbox"
                      id="nitrogenio-foliar"
                      className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                      checked={watch('nitrogenio') || false}
                      onChange={(e) => setValue('nitrogenio', e.target.checked)}
                    />
                    <label htmlFor="nitrogenio-foliar" className="ml-2 block text-sm text-gray-700">
                      Nitrogênio (R$ 15,00)
                    </label>
                  </div>
                </div>
